...
func _on_click() -> void:
	...
	var budget_code_found: bool = get_parent().get_parent().get_meta("budget_code_found", false)

	if not budget_code_found:
		await E.queue([
...
func _request_signature() -> void:
	...
	get_parent().get_parent().get_node("Props/SignatureScanner").set_meta("ready_to_sign", true)
