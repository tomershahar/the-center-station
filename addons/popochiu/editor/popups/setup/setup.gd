...
func _select_config_template() -> void:
	...
	if not _selected_template:
		var template_buttons := gui_templates.get_children().filter(
			func(c): return c is GUITemplateButton
		)
		if not template_buttons.is_empty():
			_on_gui_template_selected(template_buttons[template_buttons.size() - 1])
